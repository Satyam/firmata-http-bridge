...
| URL                                       | Response                                                                                                                | Source                                                                                           |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `http://localhost:8000`                   | web page located at `public/index.html`                                                                                 | [:octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L60-L65)       |
| `http://localhost:8000/something.txt`     | text file located at `public/something.txt`                                                                             | [:octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L67-L72)       |
| `http://localhost:8000/dist/index.js`     | Javascript file located at `dist/index.js`                                                                              | [:octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L41-L46)       |
...
A request to `http://localhost:8000`  will return the file `public/index.html` which contains an example of remote access [ :octocat: ](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L60-L65).
A request to `http://localhost:8000/someFile.txt`  will return the file `public/someFile.txt` if there is any [ :octocat: ](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L67-L72). In general, any request not fulfilled by any of the previous paths, will be tried on the `public/` folder.
...
* `dist/` is a folder containing the files compiled by the `npm run build` command.  [ :octocat: ](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L41-L46)
* `docs/` links to `docs/index.html` which is generated by the `npm run docs` command, and links to all the API docs. [ :octocat: ](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L48-L53)
* `coverage/` links to `coverage/index.html` which is generated by the `npm run coverage` command and links to the detailed coverage report for the tests. [ :octocat: ](https://github.com/Satyam/firmata-http-bridge/blob/main/src/server.ts#L54-L59)
...
When either side wants to send something to the other side, the emitter calls the `emit` method of the socket both in the [client :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/public/sockets.html#L178-L181) or in the [server :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/sockets/index.ts#L63-L72).  The first argument to the `emit` method is the *event-type*.  In hardware events, that would me `mouseclick` or `keypress`, but in software events, it is whatever the developer wants.  In this case, the messages going from client to server are `'command'` and the ones from the server to the client `'reply'` mostly as a throwback to the POST commands since in this case, the server can emit events to the client at any time, not only as a reply to some request.

On the receiving side, be it the [client :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/public/sockets.html#L184) or the [server :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/sockets/index.ts#L75) you subscribe to those events by calling the `on` method and giving the *event-type* you mean to listen to.  Just like in hardware events, where you set listeners for `onClick` or `onKeyPress`, here you set listeners for software events defined by the developer.
...
The [digitalReadSubscribe :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/pinCommands.ts#L192-L206) FSA tells the server that we want to listen to value changes on the given pin.
...
Once subscribed, the server will emit messages with the value of the pin whenever it changes.  This is done by the [callback function :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/sockets/index.ts#L62-L73) and has the same format as the FSA for the single `digitalRead` as used in the [POST `digitalRead`](#post-digitalread):
...
There is a `callback` function for each `pin` and the are all saved in [`callbacks`](https://github.com/Satyam/firmata-http-bridge/blob/my-firmata/src/sockets/index.ts#L61).  The reason for that is that in order to unsubscribe [`EventEmitter`](https://nodejs.org/docs/latest/api/events.html#events_emitter_removelistener_eventname_listener) expets the `eventName` and also the `callback`. Since many callbacks can be registered to the same event, when you remove one, you have to tell it which, as can be seen in the following action.

#### digitalReadUnsubscribe

The [digitalReadUnsubscribe :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/pinCommands.ts#L227-L245) command tells the server that we don't want pay any further attention to the given pin.
...
After importing the various pieces we need to perform the tests, we do some [basic setup :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/pinCommands.test.ts#L39-L61).  Jest makes available automatically some functions that register some code to run at specific times:
...
For example, in the [`digitalWrite` :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/pinCommands.test.ts#L86-L116) set of tests. We have tests to check when it is set high, when it is set low, when we give an incorrect pin number and when we try to set the output to an invalid value (not 0 nor 1).

Since all the tests for writing expect the pin to be set for output, we ensure that by use [`beforeAll`](https://github.com/Satyam/firmata-http-bridge/blob/my-firmata/src/pinCommands.test.ts#L87-L89).  This gets executed just once before all the tests in this *describe* block, in addition to the ones mentioned above.
...
For [`pin High` :octocat:](https://github.com/Satyam/firmata-http-bridge/blob/main/src/pinCommands.test.ts#L91-L95) we set the pin to output mode, using the `pinMode` function we tested in the previous lines.  We then use `digitalWriteActionBuilder` to build the FSA for the `digitalWrite` action. We then call [`digitalWrite` :octocat:](digitalWriteActionBuilder) with the `writeHigh` FSA we have just built.  This line is particularly important so we'll analyze it in detail:
